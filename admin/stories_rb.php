...
$ACTIVE_MENU = 'stories';
$ACTIVE_SUB  = 'rb';
$RB_STORY_TARGET = 15; // bawat color set dapat may 15 stories
...
/* ------------------------------------------------------------------
   Levels (colors) – walang thresholds para sa RB
   ------------------------------------------------------------------ */
$levels = [];
$lvlSql = "
  SELECT
...
";
if ($r = $conn->query($lvlSql)) {
  while ($row = $r->fetch_assoc()) $levels[] = $row;
}

/* ------------------------------------------------------------------
   RB sets + story count (walang kahit anong threshold logic)
   ------------------------------------------------------------------ */
$sets = [];
$sql = "
  SELECT
...
$res = $conn->query($sql);
if ($res) {
  while ($r = $res->fetch_assoc()) $sets[] = $r;
}
...
<style>
  ...
  /* Columns: # | Color | Stories | Status | Updated | Actions */
  .table-head{
    display:grid;
  ...
  .rb-page{ padding-top:60px; }
  .rb-page .page-wrap{ margin:6px auto 18px; }
...
<div class="main-content rb-page">
  <div class="page-wrap">
    ...
    <div class="sub-bar">
      <h3>RB – Story Sets (by Color)</h3>
      <div class="grow"></div>
      <input type="search" id="q" class="search" placeholder="Search color or title...">
      <button class="btn btn-accent" id="btnAddRB"><i>＋</i> Add RB Set</button>
    </div>
    ...
    <div class="card table-wrap" style="margin-top:12px;">
      ...
      <?php if(!$sets): ?>
        <div class="row">
          <div>—</div>
          <div class="muted">No RB sets yet.</div>
          <div class="muted">—</div>
          <div><span class="status st-draft">draft</span></div>
          <div class="hide-md">—</div>
          <div class="actions">
            <button class="btn btn-accent" id="btnAddRB2">＋ Add your first set</button>
          </div>
        </div>
      <?php endif; ?>
      ...
      <?php foreach($sets as $i => $s): ?>
        <div class="row rb-row">
          ...
          <div><?= (int)($s['story_count'] ?? 0) ?> / <?= (int)$RB_STORY_TARGET ?></div>
          ...
          <div class="actions">
            <a class="btn btn-manage" href="rb_manage.php?set_id=<?= (int)$s['set_id'] ?>">📚 Manage</a>

            <?php if($st!=='published'): ?>
              <form method="post" action="rb_sets_action.php" class="js-status-form" style="display:inline">
...
<!-- Add RB Set Modal -->
<div class="modal-backdrop" id="mAddRB">
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="mAddRBTitle">
    <header id="mAddRBTitle">Add RB Set</header>
    <form method="post" action="rb_sets_action.php" id="addRBForm">
      <section>
        <div class="control">
          <label>Level / Color <span style="color:#c00">*</span></label>
          <select name="level_id" id="rbLevel" required>
        ...
        <div class="control">
          ...
          <input type="text" name="title" id="rbTitle" required placeholder="e.g., RB – Yellow">
        </div>
      ...
      <footer>
        <button type="button" class="btn" id="rbClose">Cancel</button>
...
<script>
  ...
  q && q.addEventListener('input', () => {
    ...
    document.querySelectorAll('.rb-row').forEach(r=>{
  ...
  // Modal open/close
  const mRB   = document.getElementById('mAddRB');
  const openRB  = ()=> mRB.classList.add('show');
  const closeRB = ()=> mRB.classList.remove('show');
  document.getElementById('btnAddRB')?.addEventListener('click', openRB);
  document.getElementById('btnAddRB2')?.addEventListener('click', openRB);
  document.getElementById('rbClose')?.addEventListener('click', closeRB);
  mRB?.addEventListener('click', (e)=>{ if(e.target===mRB) closeRB(); });
  ...
  rbLevel?.addEventListener('change', ()=>{
    const opt = rbLevel.selectedOptions[0];
    if(opt && opt.dataset.name && (!rbTitle.value || rbTitle.value.startsWith('RB – '))){
      rbTitle.value = 'RB – ' + opt.dataset.name;
    }
  });
</script>
