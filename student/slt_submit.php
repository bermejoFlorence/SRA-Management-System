...
try {
    ...
    /**
     * 🔴 OPTION A: Guard for "invalid / unscored" attempts
     * ---------------------------------------------------
     * Halimbawa:
     *  - Wala talagang tamang sagot (0% overall)
     *  - O walang na-match na level threshold (walang color category)
     *  - O total_max = 0 (may attempt_stories pero walang items na na-score)
     *
     * Sa mga ganitong kaso:
     *  - Hindi tayo mag-aassign ng level
     *  - Itatakda natin ang attempt as 'expired' (or any status you like)
     *  - Magbabalik tayo ng allow_retake=true para ma-handle ng frontend
     */
    $MIN_VALID_PERCENT = 1; // >0; pwede mong taasan kung gusto mong may minimum performance

    $has_band          = (bool)$band;
    $has_level_id      = !empty($assigned_level_id);
    $has_items         = ($total_max > 0);
    $has_any_score     = ($total_score > 0);
    $percent_ok        = ($percent >= $MIN_VALID_PERCENT);

    $is_valid_for_level = $has_band && $has_level_id && $has_items && $has_any_score && $percent_ok;

    if (!$is_valid_for_level) {
        // Mark the attempt as "expired / unscored", walang level_id
        $conn->begin_transaction();

        $u = $conn->prepare("
            UPDATE assessment_attempts
               SET total_score = ?, total_max = ?, percent = ?, level_id = NULL,
                   status = 'expired',
                   submitted_at = NOW()
             WHERE attempt_id = ?
        ");
        $u->bind_param('iidi', $total_score, $total_max, $percent, $attempt_id);
        $u->execute();
        $u->close();
        ...
        echo json_encode([
            'ok'           => true,
            'allow_retake' => true,
            'retry_message'=> 'Your Starting Level Test was not scored (no valid responses before time ran out). You may retake the SLT.',
            'summary'      => [
    ...
    // 1) finalize attempt (valid / scored)
    $u = $conn->prepare("
        UPDATE assessment_attempts
    ...
    if ($assigned_level_id) {
        ...
        $clear = $conn->prepare("
            UPDATE student_level
               SET is_current = 0, current_flag = 0
             WHERE student_id = ?
        ");
